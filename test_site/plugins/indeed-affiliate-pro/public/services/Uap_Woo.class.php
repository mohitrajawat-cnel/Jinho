...
class Uap_Woo extends Referral_Main{
	...
	public function create_referral($order_id=0){
		...
		if(!is_admin() || (is_admin() && isset($_REQUEST['page']) && $_REQUEST['page'] == 'affiliate_import'))
		{
			...
			$uid_hwe = get_current_user_id();
			$table = $wpdb->prefix . 'uap_affiliates';

			$q = $wpdb->prepare("SELECT id,uid,rank_id,start_data,status FROM $table WHERE uid=%d ;", $uid_hwe);
	
			$exists = $wpdb->get_row($q);
	
			if($exists)
			{
				self::$affiliate_id = $exists->id;
			}
		...
		if ($this->valid_referral()){
			...
			$insert_hwe = "INSERT into custom_rank_upgrade SET user_id='".self::$user_id."',affiliate_id='".self::$affiliate_id."',amount='".$product_price_sum."'";
			...
			$this->save_referral_unverified($args,$order_id);
