...
class AffiliateAddEdit{
	...
	public function form(){
		...
		/*extra fields that must be transalted:*/
			...
			$countryArray = array(
			  ...
			  $get_all_country_name = get_option('whitelistcountrycodes');
			
			
			  $option_for_country_code='';
			  foreach($get_all_country_name as $get_all_country_name)
			  {
				   $country_name = strtoupper($get_all_country_name);
			
					foreach($countryArray as $key_country => $countryArray_hwe)
					{
						if($countryArray_hwe['name'] == $country_name)
						{
							$option_for_country_code .= '<option value="'.$countryArray_hwe['code'].'">+'.$countryArray_hwe['code'].'</option>';
						}
					}
			  }
				...
				switch ($v['name']){
					...
					default:
						...
						if ($this->is_public) {
							...
							if($v['label'] == 'Last Name')
							{
								break;
							}

							if($v['name'] == 'user_login')
							{
								//mohit
								$str .= '<div class="uap-form-line-register ' . $temp_type_class . '" id="' . $parent_id . '" style="display:flex;">';
							...
							//mohit
								if($v['label'] == 'Username')
								{
									$str .= esc_html__('Mobile Number', 'uap');
								}
								elseif($v['label'] == 'First Name')
								{
									$str .= esc_html__('Full Name', 'uap');
								}							
								else
								{
							...
							{
								//mohit
								$str .= '<span><select name="admin_user_country_code">'.$option_for_country_code.'</select></span>';
		...
		if (!$this->is_public && $indeed_db->is_magic_feat_enable('mlm') ){ //  && ($this->type=='create' || !$indeed_db->mlm_get_parent($indeed_db->get_affiliate_id_by_wpuid($this->user_id)) )
			...
			//mohit
			if(isset($_REQUEST['tab']) == 'affiliates' && isset($_REQUEST['subtab']) == 'add_edit' && (isset($_REQUEST['id']) && $_REQUEST['id'] != '') )
			{

				$affilate_id_hwe =  $mlmParent + 60000;
				...
				<script>
						jQuery(document).ready(function(){
							...
							var parent_affilater_value = jQuery("#usernames_search").val();
							if(parent_affilater_value != '')
							{
								var affiliate_id = '<?php echo $affilate_id_hwe; ?>'

							    jQuery("#usernames_search").attr("value",affiliate_id);
							}
			...
			//mohit_hwe
			//Multi Level Marketing Section updaresd to Affiliate
	...
	private function print_wp_role(){
		...
		$str .= '<div class="uap-form-line-register uap-form-select-role"><div class="uap-form-line">';

		$str .= '<h4>'.esc_html__('WordPress Role', 'uap').'</h4>';

		$str .= '<p>'.esc_html__('If is necessary choose a specific wp role for current Affiliate user', 'uap').'</p>';
	...
	private function select_rank(){
		...
		if (!$this->is_public){
			...
			$output .= '<div class="uap-form-line-register uap-form-select-ranks"><div class="uap-form-line">';

			$output .= '<h4>'.esc_html__('Affiliate Rank', 'uap').'</h4>';

			$output .= '<p>'.esc_html__('Manually assign a specific Rank based on affiliate stage or achievements', 'uap').'</p>';
	...
	private function the_slug(){
		...
		if (!$this->is_public){
			...
			$output .= '<div class="uap-form-line-register uap-form-select-slug"><div class="uap-form-line">';
		...
		if (!empty($field_arr['conditional_logic_corresp_field']) && $field_arr['conditional_logic_corresp_field']!=-1){
			...
			if ($key!==FALSE && !empty($this->register_fields[$key]['type'])){
				...
				if (!empty($js_function)){
					...
					$this->global_js .= 'jQuery(document).on( "load",function(){' . $js_function . '});';
...

	{
			...
			wp_enqueue_script( 'uap-register-captcha', UAP_URL . 'assets/js/register-captcha.js', ['jquery'], 1.1 );
	...
	private function print_overview_post_select(){
		...
		$str .= '<div class="uap-form-line-register uap-form-select-overview-post"><div class="uap-form-line">';

		$str .= '<h4>' . esc_html__('Select Post For Account Page Overview:', 'uap') . ' </h4>';

		$str .= '<p>' . esc_html__('Replace Overview tab default content with a custom one for current Affiliate user. Choose a specific WP Post content to show up there.', 'uap') . ' </p>';
		...
		foreach ($this->register_fields as $fieldId => $value){
			...
			if (isset($_REQUEST[$name])){
				...
				if ( $this->register_fields[$fieldId]['type'] == 'date' ){
						...
						$_REQUEST[$name] = preg_replace( "([^0-9-])", '', $_REQUEST[$name] );
				...
				if (!empty($value['native_wp']) ||  $name=='user_login'){
						...
						$this->fields[$name] = filter_var( $_REQUEST[$name], FILTER_SANITIZE_STRING );
						...
						if ( is_array( $_REQUEST[$name] ) ){
							...
							$custom_meta_user[$name] = indeedFilterVarArrayElements( $_REQUEST[$name] );

						} else {

							$custom_meta_user[$name] = filter_var( $_REQUEST[$name], FILTER_SANITIZE_STRING );
...
//mohit
       ...
       if($this->errors['user_email'] == 'You must enter a valid Email address.')
	   {
				...
				if (isset($_REQUEST['g-recaptcha-response'])){
					...
					if ($secret){
						...
						if (!class_exists('ReCaptcha')){
							...
							include_once UAP_PATH . 'classes/services/recaptcha/autoload.php';
	...
	private function set_mlm_parent(){
		...
		if ($this->type=='create' && $this->is_public && $indeed_db->is_magic_feat_enable('mlm') ){
			...
			if ( $_POST['uap_affiliate_mlm_parent'] === '0' || $_POST['uap_affiliate_mlm_parent_slug'] === '' ){
