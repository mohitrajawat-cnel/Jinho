<?php
if (!class_exists('MLMGetChildren')):

	class MLMGetChildren{
			...
			if ($affiliate_id){
				...
				if ($this->max_depth>0 && $children){
					...
					foreach ($children as $child_id){
						...
						$temp['avatar'] = uap_get_avatar_for_uid($childuid);
						$temp['id'] = $child_id;
		...
		private function get_childs_for_child($parent_id=0, $current_depth = 0){
			/*
			 * RECURSIVE FUNCTION
			 * @param int, int
			 * @return none
